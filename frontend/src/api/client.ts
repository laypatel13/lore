...
import type { IngestRequest, IngestStatus, QueryRequest, QueryResponse, MemoryStats, GraphApiResponse } from '../types'
...
export const api = {
  ...
  chat: {
    ...
    graph:   (repoId: string)        => http.get<GraphApiResponse>(`/chat/graph/${repoId}`).then(r => r.data),
  },
}